...
async def on_message(message):
    ...
    if message.content.startswith('!каталик шип'):
        ...
        emb.add_field(name = '  ', value= 'Скорбь... Готовли ты познать всю глубину отчаяния?' )
        emb.add_field(name= 'Условие для получения', value= 'Группой из 3 стражей отправится в Яму ереси, вооружившись оружеем скорби. Убить Зулмака Оридее Пыток ни разу не умерев.')
        emb.add_field(name= 'Задание катализатора', value= 'Убить стража в патрульной зоне, в любой точке Солнечной системы')
        emb.add_field(name= 'Бонус катализатора', value= 'Повышает дальность, стабильность. А также скрытый перк Взглять тьмы - позволяет видеть носителей света в радиусе 20 метров')
